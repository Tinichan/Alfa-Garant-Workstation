...
void __fastcall TForm6::FormActivate(TObject *Sender)
{
	...
	{
		ShowMessage(DM->IBDatabase1->DatabaseName+"Помилка: Не вдалось підключитися до БД!" );
		return;
	}
...
void TForm6::LogIn()
{
		...
		if (DM->IBQuery6->RecordCount>0)
	{
		...
		if (DM->PRESTAVNIKI_DataSet6->FieldByName("DOLSHNOST")->AsString == "Адміністратор")
		{
		...
		if (DM->PRESTAVNIKI_DataSet6->FieldByName("DOLSHNOST")->AsString == "Представник")
		{
	...
	{
		MessageDlg("Пароль невірний!",mtError, TMsgDlgButtons() << mbOK,0);
		Edit2->SetFocus();
		Edit2->Text = "";
	};
}
